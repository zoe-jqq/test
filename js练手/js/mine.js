	...
	methods: {
		...
		andThen(x, y) {
			...
			if (this.all[x][y][0] == 1 || this.all[x][y][1] > -1) {
				return;
			} /* 	// console.log("调用一次andThen" + x + ":" + y)// if(x-1<=0||x+1>=9||y-1<=0||y+1>=0){//   return;// }*/
			this.num = 0;
			try {
				...
				this.$set(this.all, x, this.all[x]) this.$set(this.all[x], y, this.all[x][y]);
		...
		randomBegin(bombNumber) {
			...
			for (let i = 0; i < this.height; i++) {
				for (let j = 0; j < this.width; j++) {
					this.$set(this.all, i, this.all[i]) this.$set(this.all[i], j, this.all[i][j]);
		...
		random() {
			...
			if (this.bomb < this.bombNum) { /* // 如果这个数炸弹，那么重新随机*/
				if (this.all[a][b][0] == 1) {
					this.random();
				} // 否则给他设置为炸弹
				else {
					this.bomb = this.bomb + 1;
					this.$set(this.all, a, this.all[a]) this.$set(this.all[a], b, this.all[a][b]);
